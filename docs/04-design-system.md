...
- 丁香紫、浅青与纯白构成品牌基础倾向，允许根据页面媒体、内容和场景扩展局部色彩与渐变，不要求所有界面维持角色专属配色。
...
- 具体字体在页面开发前结合实际媒体、文案和构图确认，不在设计阶段预先锁定。
...
- 官方角色立绘、剪影和角色专属徽标。
- 游戏截图、官方 UI、音效和动画素材。
- 来源和使用边界清楚的普通二次元场景、插画、动态图片与视频素材。
- 对官方图标、纹理和边框的描摹或改编。
...
- 外部媒体可以承担页面主视觉、画面与氛围，但不直接复制其自带 UI、字体、组件语义或文案语气；普通二次元人物可以作为页面插画使用，但不得被误写成 Purple Heart 或 Purple Sister。
- Purple Nexus 的整体 UI 采用现代二次元视觉方向，允许根据不同媒体与页面目标调整构图、色彩、装饰密度和动效。Purple Heart 与 Purple Sister 是重要主题灵感和可重复出现的内容元素，但不构成全站 UI 的唯一设计母题，也不要求每项媒体或组件复述其角色设定与作品世界观。
...
| 展开与场景转换 | 320～480ms | 页面区块和 Agent 面板 |
| 环境动画 | 2～6s | 呼吸光和能量循环 |
...
以下站点作为 Purple Nexus 的全局前端设计与动效参考库使用，不代表项目要复制其网站结构、品牌风格或页面文案，也不自动成为项目依赖：

- [Motion Reduce Bundle Size](https://motion.dev/docs/react-reduce-bundle-size)：参考 React 动效的按需加载、tree-shaking、Reduced Motion 和包体控制原则。
- [React Bits](https://reactbits.dev/)：参考 React 背景、文字效果、动画组件和可组合的视觉模式。
- [Uiverse](https://uiverse.io/)：参考按钮、卡片、状态徽标、玻璃和微交互；具体组件使用前必须核对其许可证、语义和可访问性。
- [Anime.js](https://animejs.com/)：参考时间线、easing、stagger、SVG 和响应式动画的 API 思路；不与现有动画系统无理由并行引入。
- [Aceternity UI](https://ui.aceternity.com/)：参考 Hero、背景、光束、卡片、Bento 和微交互的组合方式，不照搬整站布局。

采用外部参考时必须先判断实际职责、包体成本、许可证、键盘行为、Reduced Motion 和当前主题 Token 的适配边界；能够用现有 CSS、GSAP 或平台能力完成时，不为了追求视觉相似而新增重复依赖。
